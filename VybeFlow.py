...
# Import config and posts_api blueprint
...
# Reel model (stub so references don't break)
class Reel(db.Model):
    id = db.Column(db.Integer, primary_key=True)
...
class Post(db.Model):
    ...
    def media_path(self):
        if not self.image_filename:
            return None
        if self.image_filename.startswith("uploads/"):
            return self.image_filename
        return f"uploads/media/{self.image_filename}"
...
def register():
    if request.method == 'POST':
        ...
        user.bio = 'VybeFlow member.'
        user.avatar_url = url_for('static', filename='VFlogo_clean.png')
...
def upload():
    ...
    if request.method == 'POST':
        ...
        try:
            post = Post()
            post.image_filename = f"uploads/media/{filename}"
...
def settings():
    ...
    if request.method == 'POST':
        ...
        if avatar_file and avatar_file.filename:
            ...
            media_dir = os.path.join(app.config['UPLOAD_FOLDER'], 'media')
            os.makedirs(media_dir, exist_ok=True)
            avatar_file.save(os.path.join(media_dir, filename))
            user.avatar_url = url_for('static', filename=f'uploads/media/{filename}')
...
if __name__ == '__main__':
    ...
    for folder in [app.config['UPLOAD_FOLDER'], app.config.get('POST_UPLOAD_ABS')]:
